...
**SMTP 与部署网址不用写在 config.yaml**——在 Web 界面配置:登录管理员 → 系统设置 → 邮件通知 tab:
- **部署网址(必填)**:服务器访问地址,如 `http://<服务器IP>:8081`。邮件会附带此地址作为登录入口;**未填写时不发送任何邮件**。
- **SMTP 配置**:服务器地址、端口、认证、发件人(内网 SMTP),有"测试发送"按钮验证。

邮件功能(到期提醒、密码重置/临时密码)依赖以上两项配置。
