...
- **[2026-08-21] 计划/实际日期联动体系(用户拍板,大交付)**：四者并存(计划日期/计划工期/实际日期/实际工期)。①计划锚点编辑=写现有约束(正排改计划开始→SNET,倒排改计划结束→FNLT),约束是单值列→锚点与约束选择器互锁(FNLT 态下计划开始禁编辑);②实际日期默认空白(删除 fillActualDates 自动填充),实际工期派生显示+落库 actual_duration_days(migration v10,后端 UpdateTask 权威计算,前端不发该字段);③已完成任务引擎级冻结(所有 Recalculate 路径跳过 completed,但链条穿透传播);④backwardScheduleWrite 原本不读 FNLT——倒排锚定是硬前提,P3 加 FNLT 锚定(锚点任务入队不写回);⑤甘特图页"重新计算排程"按钮(POST /api/projects/{id}/reschedule → RecalculateAll → WS 广播);⑥弹窗两栏新布局(左:基本信息+前置任务,右:日期与进度+状态+约束)。
- **[2026-08-21] 工作日语义三函数口径**：AddWorkDays/SubWorkDays **不含起点**(8/3 起 3 工作日 → end=8/6),CountWorkDays **含首尾**(CountWorkDays(8/3,8/6)=4)。前端 date.ts 的 addWorkDays/subWorkDays/countWorkDays 镜像后端,含节假日日历(GET /api/calendar,calMap date→holiday/workday)。实际工期用 CountWorkDays(含首尾),排程 duration 用 AddWorkDays(不含起点)——二者差 1 天是正常语义,不是 bug。
- **[2026-08-21] 任务条拖拽已禁用**：ProjectGantt `drag_move=false; drag_resize=false`(仅 drag_links 和行排序可拖)——manual_scheduled 已无任何入口用途(弹窗 checkbox 是摆设),彻底废弃(引擎跳过逻辑删除、API/DB 列保留防迁移)。
- **[2026-08-21] gitnexus detect_changes 报 FTS 缺失**：`query` 工具返回 "FTS indexes missing — keyword search degraded",需 `npx gitnexus analyze --force` 重建关键词索引;detect_changes 本身可用但索引是旧的(本次 105 符号变更含 TaskInfo.ManualScheduled 等已删符号残留)。收工前应跑 analyze 刷新索引。
- **[2026-08-21] 看板占满视口的实现模式(用户拍板"对了")**：全局 `.main-content` max-width 1440 居中,页面要占满视口必须像甘特图页一样突破——Dashboard 挂载时 `useEffect` 给 `.main-content` 动态加 `dashboard-page` class(卸载移除),index.css 加 `.main-content.dashboard-page { max-width: none }`,`.dashboard` 本身 `width: 100%`(main 的 padding 24px 32px 保留作边距)。**别改 `.dashboard` 的 max-width/margin——父容器限制才是根因**(之前 1200/1400 居中、flex 加宽全失败,就是这个)。
- **[2026-08-21] 用户显示器是 1920(推翻 1536 假设)**：用户明确"我的显示器分辨率 1920"——cerebrum 旧记录"有效视口 1536x960(125% 缩放)"已过时/不适用于当前显示器。**1920 下 1200px 居中容器左右各 360px 空白,用户不可接受;布局验证一律以 1920x1080 为基准**,除非用户说明窗口非全屏。
...
- **[2026-08-21] INSERT 加列后四段核对（bug-275）**：改 INSERT 列清单(加/删列)后必须四段对齐核对：**列数 = 表达式数 = ? 数 = 参数数**。只数 ? 不够——固定值(0/''/'medium')也占列位；modernc sqlite 报 "N values for M columns"/"missing argument with index N" 时先数四段,别急着加 log(本次 CreateTask 20 表达式/19 列、ImportTasks 少一参数,各耗一轮调试)。
