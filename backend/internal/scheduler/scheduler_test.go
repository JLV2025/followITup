...
func TestForwardPass(t *testing.T) {
	...
	changes := forwardPass(tasks, deps, 1, map[int64]bool{}, map[string]string{})
	if len(changes) != 2 {
...
func TestStartNoEarlierThanConstraint(t *testing.T) {
	...
	// A 提前到 7/3 完成，候选 7/4 → 约束 floor 7/20 → 候选=7/20, 7/20 <= 7/25 → 不更新
	tasks[0].EndDate = "2026-07-03"
	tasks[0].DurationDays = 2

	changes := forwardPass(tasks, deps, 1, map[int64]bool{}, map[string]string{})
	if _, ok := changes[2]; ok {
		t.Error("B 的约束 floor(7/20) ≤ 当前(7/25)，不应被改变")
	}

	// 但如果 A 延期到 7/28，候选=7/29 > B 当前 7/25 → 应推送
	tasks[0].EndDate = "2026-07-28"
	tasks[0].DurationDays = 27
	changes2 := forwardPass(tasks, deps, 1, map[int64]bool{}, map[string]string{})
...
func TestBackwardPassDeadline(t *testing.T) {
	...
	forwardPass(tasks, deps, 1, map[int64]bool{}, map[string]string{})
